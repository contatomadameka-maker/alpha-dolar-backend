...
STRATEGY_MAP = {
    ...
    'alpha_regente':      lambda tm, rm: AlphaRegente(tm, rm),
}
...
def start_bot():
    ...
    try:
        ...
        _strategy_preview = 'alpha_perfil' if bot_type == 'perfil' else ('alpha_regente' if bot_type == 'regente' else config.get('strategy', 'alpha_bot_1'))
        print(f"🎯 Estratégia: {_strategy_preview} ({config.get('strategy_type','').upper() or 'RISE/FALL'})")
        ...
        # Impede reiniciar o MESMO tipo de bot que ja esta rodando --
        # tipos diferentes agora podem rodar em paralelo (Alpha Esquadrao),
        # já que a configuracao de cada um é isolada por instancia (self.config).
        ...
        ENXAME_SLOTS_TESTE = [f'enxame-{i}' for i in range(1, 21)]  # temporario, so para teste de carga (ate 20)
        if BOTS_AVAILABLE and bot_type in ['ia', 'ia_simples', 'perfil', 'regente'] + ESQUADRAO_SLOTS + ENXAME_SLOTS_TESTE:
            ...
            strategy_id    = 'alpha_perfil' if bot_type == 'perfil' else ('alpha_regente' if bot_type == 'regente' else config.get('strategy', 'alpha_bot_1'))
...
def get_bot_stats(bot_type):
    ...
    return jsonify({
        ...
        'regime_atual': getattr(getattr(bot, 'strategy', None), 'ultimo_regime', None) if bot else None,
    })
