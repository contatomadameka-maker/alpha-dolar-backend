"""
Alpha Bot 2 - Reversão de Tendência
FREE | Win Rate esperado: 74%
Lógica: Detecta sobrecompra/sobrevenda e entra na reversão
"""
from .base_strategy import BaseStrategy
from ..config import BotConfig
import statistics

class AlphaBot2(BaseStrategy):
    TRADING_MODE_CONFIG = {
        'lowRisk':  {'min_confidence': 0.90, 'cooldown': 22, 'min_conditions': 4},
        'accurate': {'min_confidence': 0.80, 'cooldown': 16, 'min_conditions': 4},
        'balanced': {'min_confidence': 0.70, 'cooldown': 11, 'min_conditions': 3},
        'faster':   {'min_confidence': 0.60, 'cooldown':  7, 'min_conditions': 3},
    }
    RISK_MODE_CONFIG = {
    ...
    def __init__(self, trading_mode='faster', risk_mode='conservative'):
        super().__init__(name="Alpha Bot 2")
        self.min_history = 20
        ...
        print(f"⚙️ Alpha Bot 2 | Modo: {trading_mode} | Confiança: {self.min_confidence:.0%}")
    ...
    def should_enter(self, tick_data):
        ...
        try:
            ...
            # Sequência dos últimos 5 ticks (direção)
            last5 = prices[-5:]
            consec_up   = all(last5[i] > last5[i-1] for i in range(1, 5))
            consec_down = all(last5[i] < last5[i-1] for i in range(1, 5))

            momentum = (prices[-1] - prices[-5]) / prices[-5] * 100

            # CALL: preço bateu na banda inferior e começa a reverter
            call_conditions = [
                current <= lower_band,
                consec_down,
                momentum < -0.1,
            ...
            # PUT: preço bateu na banda superior e começa a reverter
            put_conditions = [
                current >= upper_band,
                consec_up,
                momentum > 0.1,
    ...
    def get_info(self):
        return {'name': self.name, 'tier': 'FREE', 'trading_mode': self.trading_mode,
                'risk_mode': self.risk_mode, 'win_rate': '74%'}
